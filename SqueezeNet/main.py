...
from modified_squeezenet import ModifiedSqueezeNet  # Assuming your modified model is in this file


# Argument parsing
parser = argparse.ArgumentParser(description='Train SqueezeNet on CIFAR-10')
parser.add_argument('--batch-size', type=int, default=64)
parser.add_argument('--epoch', type=int, default=25)  # Increased epochs#100
parser.add_argument('--learning-rate', type=float, default=0.001)
parser.add_argument('--momentum', type=float, default=0.9)
parser.add_argument('--no-cuda', action='store_true', default=False)
parser.add_argument('--log-schedule', type=int, default=10)
parser.add_argument('--seed', type=int, default=1)
parser.add_argument('--model_name', type=str, default=None)
parser.add_argument('--want_to_test', action='store_true', default=False)
parser.add_argument('--epoch_55', action='store_true')
...
def train(epoch):
    if args.epoch_55:
        params = paramsforepoch(epoch)
        print(f"Configuring optimizer with lr={params['learning_rate']} and weight_decay={params['weight_decay']}")
        adjustlrwd(params)
    ...
    for b_idx, (data, targets) in enumerate(train_loader):
        if args.cuda:
            data, targets = data.cuda(), targets.cuda()
        data, targets = torch.autograd.Variable(data), torch.autograd.Variable(targets)
        ...
        if b_idx % args.log_schedule == 0:
            print(f'Train Epoch: {epoch} [{(b_idx+1) * len(data)}/{len(train_loader.dataset)} ({100. * (b_idx+1) * len(data) / len(train_loader.dataset):.0f}%)]\tLoss: {loss.item():.6f}')
            plt.plot(avg_loss)
            plt.savefig("Squeezenet_loss.jpg")

    train_accuracy = correct / float(len(train_loader.dataset))
    print(f"Training accuracy ({100 * train_accuracy:.2f}%)")
    return 100.0 * train_accuracy
...
def validate():
    ...
    if val_accuracy > best_accuracy:
        best_accuracy = val_accuracy
        torch.save(net.state_dict(), 'squeezenet_onfulldata.pth')
    return val_accuracy

def test():
    weights = torch.load('squeezenet_onfulldata.pth')
    net.load_state_dict(weights)
    net.eval()
    ...
    accuracy = 100.0 * test_correct / total_examples
    print(f"Testing accuracy on CIFAR-10 data is {accuracy:.2f}%")
    return accuracy

if __name__ == '__main__':
    best_accuracy = 0.0  # Initialize global variable
    if not args.want_to_test:
        train_acc, val_acc = [], []
        for epoch in range(1, args.epoch + 1):
            train_acc.append(train(epoch))
            val_acc.append(validate())
            plt.figure()
        ...
        test_acc = test()
        print(f"Testing accuracy on CIFAR-10 data is {test_acc:.2f}%")
