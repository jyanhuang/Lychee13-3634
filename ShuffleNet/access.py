...
transform = transforms.Compose([
    ...
    transforms.ToTensor(),
    transforms.Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5]),  # ImageNet的均值和标准差
])

# 使用ImageFolder自动加载数据
data_dir = 'F:/zhenghanling/Resnet-152/data/test'  # 替换为你的数据集根目录
dataset = datasets.ImageFolder(root=data_dir, transform=transform)
data_loader = DataLoader(dataset, batch_size=64, shuffle=False)

# 加载预训练的ResNet-50模型，但不包括fc层
model = models.shufflenet_v2(pretrained=True)
...
# 加载你自己的模型权重
model_weights = torch.load('shufflenet_v2.pth')
